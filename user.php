...
<?php
		if( $snippets_list )
		{
			foreach( $snippets_list as $k => $v )
			{
				...
				echo '<li><span class="titler"><a href="' . $v["id"] . '">' . $sname . '</a> by ' . $nname . '</span> <span class="langr">' . $langs["names"][ $v["language"] ] . ', ' . $v["time"] . '</span><img class=".icon-remove"><a href="./delete?"'.$v["id"].'</a></img></li>';
			}
		}
	}
?>
